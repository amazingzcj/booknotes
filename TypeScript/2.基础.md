...
```tsx
// 整体导入const foo = require('foo');
// 单个导入const bar = require('foo').bar;
```
...
// 单个导入import { bar } from 'foo';
```

第三种方式是 `import ... require`，这也是 ts 官方推荐的方式：

```tsx
// 整体导入import foo = require('foo');
// 单个导入import bar = foo.bar;
```
...
```tsx
// src/index.ts
import * as moment from 'moment';import 'moment-plugin';
moment.foo();
```
...
```tsx
// lib/index.d.ts
export * from './bar';export default function foo(): string;
```
